...
void HPCVirtDecorator::initialize(const HeterogeneousMap &params) {
  ...
  if (shots >= 1 && isVqeMode || shots < 1 && !isVqeMode) {
    ...
    xacc::info("Running shot-based simulation.");
  } else {
...
void HPCVirtDecorator::execute(
  ...
  // get expectation values/bitstrings and the size of the key of each child
  // buffer
  ...
  if (world_rank == qpuComm->getProcessRanks()[0]) {
    ...
    zeroRanksComm->allGatherv(localKeySizes, globalKeySizes, nLocalChildren,
                              nKeyShift);

    if (isVqeMode) {
      // gather all expectation values
      zeroRanksComm->allGatherv(localExpVals, globalExpVals, nLocalChildren,
                                nKeyShift);
    } else {
      // gather all bitstrings
      zeroRanksComm->allGatherv(localNumberBitStrings, globalNumberBitStrings,
                                nLocalChildren, nKeyShift);
    }
  }
  ...
  auto nGlobalBitStrings = std::accumulate(globalNumberBitStrings.begin(),
                                           globalNumberBitStrings.end(), 0);
  ...
  if (world_rank == qpuComm->getProcessRanks()[0]) {
    ...
    for (auto child : my_buffer->getChildren()) {
      ...
      if (!isVqeMode) {
        for (auto &count : child->getMeasurementCounts()) {
          ...
          // stoi is MSB
          if (decoratedAccelerator->getBitOrder() ==
              Accelerator::BitOrder::LSB) {
    ...
    zeroRanksComm->allGatherv(localKeys, globalKeyChars, commKeySize,
                              keySizeShift);
    ...
    if (!isVqeMode) {
      ...
      for (int i = 0; i < n_virtual_qpus; i++) {
        ...
        commNumberBitStrings[i] =
            std::accumulate(it, it + nLocalChildren[i], 0);
        shift += nLocalChildren[i];
      }
      ...
      for (int i = 1; i < n_virtual_qpus; i++) {
        bitStringShift[i] = std::accumulate(
            commNumberBitStrings.begin(), commNumberBitStrings.begin() + i, 0);
      }

      // gather all bit strings
      zeroRanksComm->allGatherv(localBitStringIndices, globalBitStrings,
                                commNumberBitStrings, bitStringShift);
      // gather all counts
      zeroRanksComm->allGatherv(localCounts, globalCounts, commNumberBitStrings,
                                bitStringShift);
    }
  }
  ...
  for (int i = 0; i < nGlobalChildren; i++) {
    ...
    if (isVqeMode) {
      ...
      for (int b = 0; b < nChildBitStrings; b++) {
        ...
        auto nMeasuredBits =
            std::count_if(name.begin(), name.end(), [](char c) {
              return std::string("XYZ").find(c) != std::string::npos;
            });
...
public:
  virtual void tearDown() override {
    auto c = xacc::getService<xacc::AcceleratorDecorator>("hpc-virtualization",
                                                          false);
    if (c) {
      auto casted =
          std::dynamic_pointer_cast<xacc::quantum::HPCVirtDecorator>(c);
      assert(casted);
      casted->finalize();
    }
